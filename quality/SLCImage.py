...
class SLCImage(object):
    ...
    def calc(self):
        ...
        self.avg_power = np.empty(self.shape[1:], np.float64)
        batchsize = 1024
        for b in range(0, self.shape[1], batchsize):
            self.avg_power[b:b+batchsize] = np.mean(np.abs(self.fft[:, b:b+batchsize]), axis=0)
        ...
        for irow in range(1, 2):  # don't plot real and imag images
            for icol in range(0, 2):
                ...
                if ("Power" in images[(irow, icol)][1]):
                    ...
                    xdata2, _, _ = self.clip_image(xdata)
    ...
    def clip_image(self, data_in, percent=90.0):
        ...
        return data_in2, vmin, vmax

    def scale_image(self, data_in, percent=90.0, exp=0.5):
        
        data_in2, vmin, vmax = self.clip_image(data_in, percent)
