...
Each QA HDF5 file includes a subset of the available options below, which will correspond to the available frequencies, polarizations, etc. in the input RIFG product.
...
|     | RIFG QA HDF5 Dataset, Attributes, and additional metadata |
| :---: | --------------------------------------- |
...
|    | _description:_ Basis image for the browse PNG. 'phase' if wrapped phase, 'hsi' if HSI image with phase information encoded as Hue and coherence encoded as Intensity. |
|    | _units:_ 1 |
|    | _dtype:_ fixed-length byte string |
|    | _ndim:_ scalar |
