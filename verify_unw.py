...
from quality.UNWFile import UNWFile
...
if __name__ == "__main__":
    ...
    for slc_file in args:
        ...
        fhdf = UNWFile(slc_file, xml_tree=xml_tree, mode="r")
        ...
        print("Getting F/P list")
        fhdf.get_freq_pol()

        try:
            fhdf.check_freq_pol(fgroups1=[fhdf.SWATHS], \
                                fgroups2=[fhdf.FREQUENCIES_SWATH], \
                                fnames2=["Swaths"])
        except errors_base.FatalError:
            print("File %s has a Fatal Error" % slc_file)
            fhdf.close()
        ...
        if (kwds["validate"]):
            try:
                fhdf.check_frequencies(fhdf.FREQUENCIES_SWATH)
            except errors_base.WarningError:
                pass
